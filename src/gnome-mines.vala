...
public class Mines : Gtk.Application
{
    ...
    protected override void startup ()
    {
        ...
        history = new History (Path.build_filename (Environment.get_user_data_dir (), "gnome-mines", "history"),Path.build_filename (Environment.get_user_data_dir (), "gnome-mines", "stat"));
        history.load ();
    }
    ...
    private void cleared_cb (Minefield minefield)
    {
		...
		history.addWin(minefield.width,minefield.height,minefield.n_mines);
        history.add (entry);
        history.save ();
...
public class ScoreDialog : Gtk.Dialog
{
	...
	private Gtk.Label wins_label;
	private Gtk.Label loses_label;
    ...
    public ScoreDialog (History history, HistoryEntry? selected_entry = null, bool show_quit = false)
    {
        ...
        if (size_combo.get_active_iter (out iter)) {
             size_combo.model.get (iter, 1, out width, 2, out height, 3, out n_mines);
			 StatEntry? stat=history.find_stat(width,height,n_mines);
			 if (stat!=null) {
				 wins=stat.wins;
				 loses=stat.loses;
				 total=wins+loses;
